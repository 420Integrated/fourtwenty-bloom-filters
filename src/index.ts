...
/**
 * Returns true if the 420coin users address is part of the given bloom.
 * note: false positives are possible.
 * @param bloom encoded bloom
 * @param address the address to test
 */
export function isUserFourtwentyAddressInBloom(
  bloom: string,
  fourtwentyAddress: string,
): boolean {
  if (!isBloom(bloom)) {
    throw new Error('Invalid bloom given');
  }

  if (!isAddress(fourtwentyAddress)) {
    throw new Error(`Invalid fourtwenty address given: "${fourtwentyAddress}"`);
  }

  // you have to pad the fourtwenty address to 32 bytes
  // else the bloom filter does not work
  // this is only if your matching the USERS
  // fourtwenty address. Contract address do not need this
  // hence why we have 2 methods
  // (0x is not in the 2nd parameter of padleft so 64 chars is fine)
  const address = padLeft(fourtwentyAddress, 64);
